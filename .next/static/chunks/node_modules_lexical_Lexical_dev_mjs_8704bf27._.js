 ...
 */ function $getState(node, stateConfig) {
 ...
 */ function $checkCollision(node, stateConfig, state) {
    {
        ...
        if (collision !== undefined && collision !== stateConfig) {
            {
                formatDevErrorMessage("$setState: State key collision ".concat(JSON.stringify(stateConfig.key), " detected in ").concat(node.constructor.name, " node with type ").concat(node.getType(), " and key ").concat(node.getKey(), ". Only one StateConfig with a given key should be used on a node."));
            }
        }
    }
}
   ...
   */ constructor(node, sharedConfigMap = new Map(), unknownState = undefined, knownState = new Map(), size = undefined){
        ...
        const computedSize = size !== undefined ? size : computeSize(sharedConfigMap, unknownState, knownState);
        {
            if (!(size === undefined || computedSize === size)) {
                formatDevErrorMessage("NodeState: size != computedSize (".concat(String(size), " != ").concat(String(computedSize), ")"));
            }
            for (const stateConfig of knownState.keys()){
                if (!sharedConfigMap.has(stateConfig.key)) {
                    formatDevErrorMessage("NodeState: sharedConfigMap missing knownState key ".concat(stateConfig.key));
                }
            }
        }
        ...
        if (prevFirstChildKey === nextFirstChildKey) {
            ...
            try {
                ...
                if (typeof error === 'object' && error != null) {
                    const msg = "".concat(error.toString(), " Parent: ").concat(dom.tagName, ", new child: {tag: ").concat(replacementDOM.tagName, " key: ").concat(nextFirstChildKey, "}, old child: {tag: ").concat(lastDOM.tagName, ", key: ").concat(prevFirstChildKey, "}.");
                    throw new Error(msg);
                } else {
                    throw error;
                }
...
function getPrevElementByKeyOrThrow(key) {
    ...
    if (element === undefined) {
        {
            formatDevErrorMessage("Reconciliation: could not find DOM element for node key ".concat(key));
        }
    }
    return element;
}
...
class LexicalNode {
   ...
   */ static getType() {
        {
            formatDevErrorMessage("LexicalNode: Node ".concat(this.name, " does not implement .getType()."));
        }
    }
    /**
   ...
   */ static clone(_data) {
        {
            formatDevErrorMessage("LexicalNode: Node ".concat(this.name, " does not implement .clone()."));
        }
    }
    /**
   ...
   */ getType() {
    ...
    isInline() {
        {
            formatDevErrorMessage("LexicalNode: Node ".concat(this.constructor.name, " does not implement .isInline()."));
        }
    }
    /**
   ...
   */ getParentOrThrow() {
        ...
        if (parent === null) {
            {
                formatDevErrorMessage("Expected node ".concat(this.__key, " to have a parent."));
            }
        }
        return parent;
    }
   ...
   */ getTopLevelElementOrThrow() {
        ...
        if (parent === null) {
            {
                formatDevErrorMessage("Expected node ".concat(this.__key, " to have a top parent element."));
            }
        }
        return parent;
    }
   ...
   * */ static importJSON(_serializedNode) {
        {
            formatDevErrorMessage("LexicalNode: Node ".concat(this.name, " does not implement .importJSON()."));
        }
    }
    /**
   ...
   * */ insertAfter(nodeToInsert) {
        let restoreSelection = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : true;
        errorOnReadOnly();
...
function errorOnTypeKlassMismatch(type, klass) {
    ...
    if (registeredNode === undefined) {
        {
            formatDevErrorMessage("Create node: Attempted to create node ".concat(klass.name, " that was not configured to be used on the editor."));
        }
    }
    const editorKlass = registeredNode.klass;
    if (editorKlass !== klass) {
        {
            formatDevErrorMessage("Create node: Type ".concat(type, " in node ").concat(klass.name, " does not match registered node ").concat(editorKlass.name, " with the same type"));
        }
    }
}
/**
    ...
    set(key, offset, type, onlyIfChanged) {
        ...
        this.type = type;
        {
            ...
            if (!(type === 'text' ? $isTextNode(node) : $isElementNode(node))) {
                formatDevErrorMessage("PointType.set: node with key ".concat(key, " is ").concat(node ? node.__type : '[not found]', " and can not be used for a ").concat(type, " point"));
            }
        }
...
class RangeSelection {
   ...
   */ getNodes() {
        ...
        const nodes = $getNodesFromCaretRangeCompat(range);
        {
            if (this.isCollapsed() && nodes.length > 1) {
                {
                    formatDevErrorMessage("RangeSelection.getNodes() returned ".concat(String(nodes.length), " > 1 nodes in a collapsed selection"));
                }
            }
        }
        if (!isCurrentlyReadOnlyMode()) {
   ...
   */ formatText(formatType) {
        let alignWithFormat = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : null;
        if (this.isCollapsed()) {
   ...
   */ insertNodes(nodes) {
        ...
        if (!nodes.some(notInline)) {
            if (!$isElementNode(firstBlock)) {
                formatDevErrorMessage("Expected node ".concat(firstNode.constructor.name, " of type ").concat(firstNode.getType(), " to have a block ElementNode ancestor"));
            }
            const index = $removeTextAndSplitBlock(this);
            firstBlock.splice(index, 0, nodes);
            last.selectEnd();
        ...
        if (firstToInsert) {
            if (!(firstBlock !== null)) {
                formatDevErrorMessage("Expected node ".concat(firstNode.constructor.name, " of type ").concat(firstNode.getType(), " to have a block ancestor"));
            }
...
function $validatePoint(editor, name, point) {
    ...
    if (!(node !== undefined)) {
        formatDevErrorMessage("$validatePoint: ".concat(name, " key ").concat(point.key, " not found in current editorState"));
    }
    if (point.type === 'text') {
        if (!$isTextNode(node)) {
            formatDevErrorMessage("$validatePoint: ".concat(name, " key ").concat(point.key, " is not a TextNode"));
        }
        const size = node.getTextContentSize();
        if (!(point.offset <= size)) {
            formatDevErrorMessage("$validatePoint: ".concat(name, " point.offset > node.getTextContentSize() (").concat(String(point.offset), " > ").concat(String(size), ")"));
        }
    } else {
        if (!$isElementNode(node)) {
            formatDevErrorMessage("$validatePoint: ".concat(name, " key ").concat(point.key, " is not an ElementNode"));
        }
        const size = node.getChildrenSize();
        if (!(point.offset <= size)) {
            formatDevErrorMessage("$validatePoint: ".concat(name, " point.offset > node.getChildrenSize() (").concat(String(point.offset), " > ").concat(String(size), ")"));
        }
    }
}
...
function $updateElementSelectionOnCreateDeleteNode(selection, parentNode, nodeOffset) {
    let times = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : 1;
    const anchor = selection.anchor;
    const focus = selection.focus;
    const anchorNode = anchor.getNode();
    const focusNode = focus.getNode();
 ...
 */ function $modifySelectionAroundDecoratorsAndBlocks(selection, alter, isBackward, granularity) {
    let mode = arguments.length > 4 && arguments[4] !== void 0 ? arguments[4] : 'decorators-and-blocks';
...
function getActiveEditorState() {
    if (activeEditorState === null) {
        {
            formatDevErrorMessage("Unable to find an active editor state. State helpers or node methods can only be used synchronously during the callback of editor.update(), editor.read(), or editorState.read().".concat(collectBuildInformation()));
        }
    }
...
function getActiveEditor() {
    if (activeEditor === null) {
        {
            formatDevErrorMessage("Unable to find an active editor. This method can only be used synchronously during the callback of editor.update() or editor.read().".concat(collectBuildInformation()));
        }
    }
    return activeEditor;
}
function collectBuildInformation() {
    ...
    let output = " Detected on the page: ".concat(compatibleEditors, " compatible editor(s) with version ").concat(thisVersion);
    if (incompatibleEditors.size) {
        output += " and incompatible editors with versions ".concat(Array.from(incompatibleEditors).join(', '));
    }
    return output;
}
...
function $parseSerializedNodeImpl(serializedNode, registeredNodes) {
    ...
    if (registeredNode === undefined) {
        {
            formatDevErrorMessage('parseEditorState: type "'.concat(type, '" + not found'));
        }
    }
    const nodeClass = registeredNode.klass;
    if (serializedNode.type !== nodeClass.getType()) {
        {
            formatDevErrorMessage("LexicalNode: Node ".concat(nodeClass.name, " does not implement .importJSON()."));
        }
    }
...
class ElementNode extends LexicalNode {
    ...
    getFirstChildOrThrow() {
        ...
        if (firstChild === null) {
            {
                formatDevErrorMessage("Expected node ".concat(this.__key, " to have a first child."));
            }
        }
        return firstChild;
    }
    ...
    getLastChildOrThrow() {
        ...
        if (lastChild === null) {
            {
                formatDevErrorMessage("Expected node ".concat(this.__key, " to have a last child."));
            }
        }
        return lastChild;
    }
   ...
   */ getFormatFlags(type, alignWithFormat) {
    ...
    append() {
        for(var _len = arguments.length, nodesToAppend = new Array(_len), _key = 0; _key < _len; _key++){
            nodesToAppend[_key] = arguments[_key];
        }
    ...
    splice(start, deleteCount, nodesToInsert) {
        ...
        if (!(start + deleteCount <= oldSize)) {
            formatDevErrorMessage("ElementNode.splice: start + deleteCount > oldSize (".concat(String(start), " + ").concat(String(deleteCount), " > ").concat(String(oldSize), ")"));
        }
    ...
    exportDOM(editor) {
        ...
        if (isHTMLElement(element)) {
            ...
            if (indent > 0) {
                ...
                element.style.paddingInlineStart = "".concat(indent * 40, "px");
            }
...
function exportNodeToJSON(node) {
    ...
    if (serializedNode.type !== nodeClass.getType()) {
        {
            formatDevErrorMessage("LexicalNode: Node ".concat(nodeClass.name, " does not match the serialized type. Check if .exportJSON() is implemented and it is returning the correct type."));
        }
    }
    if ($isElementNode(node)) {
        ...
        if (!Array.isArray(serializedChildren)) {
            {
                formatDevErrorMessage("LexicalNode: Node ".concat(nodeClass.name, " is an element but .exportJSON() does not have a children array."));
            }
        }
            ...
            // Ensure custom nodes implement required methods and replaceWithKlass is instance of base klass.
            {
                ...
                if (replaceWithKlass) {
                    if (!(replaceWithKlass.prototype instanceof klass)) {
                        formatDevErrorMessage("".concat(replaceWithKlass.name, " doesn't extend the ").concat(name));
                    }
                }
...
class LexicalEditor {
   ...
   */ registerCommand(command, listener, priority) {
        ...
        if (listenersInPriorityOrder === undefined) {
            {
                formatDevErrorMessage("registerCommand: Command ".concat(String(command), " not found in command map"));
            }
        }
   ...
   */ registerMutationListener(klass, listener, options) {
    ...
    /** @internal */ getRegisteredNode(klass) {
        ...
        if (registeredNode === undefined) {
            {
                formatDevErrorMessage("Node ".concat(klass.name, " has not been registered. Ensure node has been passed to createEditor."));
            }
        }
...
function getRegisteredNodeOrThrow(editor, nodeType) {
    ...
    if (registeredNode === undefined) {
        {
            formatDevErrorMessage("registeredNode: Type ".concat(nodeType, " not found"));
        }
    }
    return registeredNode;
}
...
function errorOnNodeKeyConstructorMismatch(node, existingKey) {
    ...
    if (existingNode && existingNode.constructor !== node.constructor) {
        ...
        if (node.constructor.name !== existingNode.constructor.name) {
            {
                formatDevErrorMessage("Lexical node with constructor ".concat(node.constructor.name, " attempted to re-use key from node in active editor state with constructor ").concat(existingNode.constructor.name, ". Keys must not be re-used when the type is changed."));
            }
        } else {
            {
                formatDevErrorMessage("Lexical node with constructor ".concat(node.constructor.name, " attempted to re-use key from node in active editor state with different constructor with the same name (possibly due to invalid Hot Module Replacement). Keys must not be re-used when the type is changed."));
            }
        }
    }
}
...
function setNodeKeyOnDOMNode(dom, editor, key) {
    const prop = "__lexicalKey_".concat(editor._key);
    dom[prop] = key;
}
...
function setMutatedNode(mutatedNodes, registeredNodes, mutationListeners, node, mutation) {
    ...
    if (registeredNode === undefined) {
        {
            formatDevErrorMessage("Type ".concat(nodeType, " not in registeredNodes"));
        }
    }
...
function $applyNodeReplacement(node) {
    ...
    const registeredNode = editor._nodes.get(nodeType);
    if (!(registeredNode !== undefined)) {
        formatDevErrorMessage("$applyNodeReplacement node ".concat(node.constructor.name, " with type ").concat(nodeType, ' must be registered to the editor. You can do this by passing the node class via the "nodes" array in the editor config.'));
    }
    ...
    if (replace !== null) {
        ...
        if (replaceWithKlass !== null) {
            if (!(replacementNode instanceof replaceWithKlass)) {
                formatDevErrorMessage("$applyNodeReplacement failed. Expected replacement node to be an instance of ".concat(replaceWithKlass.name, " with type ").concat(replaceWithKlass.getType(), " but returned ").concat(replacementNodeKlass.name, " with type ").concat(replacementNodeKlass.getType(), " from original node ").concat(node.constructor.name, " with type ").concat(nodeType));
            }
        } else {
            if (!(replacementNode instanceof node.constructor && replacementNodeKlass !== node.constructor)) {
                formatDevErrorMessage("$applyNodeReplacement failed. Ensure replacement node ".concat(replacementNodeKlass.name, " with type ").concat(replacementNodeKlass.getType(), " is a subclass of the original node ").concat(node.constructor.name, " with type ").concat(nodeType, "."));
            }
        }
...
function $getNodeByKeyOrThrow(key) {
    ...
    if (node === null) {
        {
            formatDevErrorMessage("Expected node with key ".concat(key, " to exist but it's not in the nodeMap."));
        }
    }
    return node;
}
 ...
 */ function $cloneWithProperties(latestNode) {
    ...
    mutableNode.afterCloneFrom(latestNode);
    {
        if (!(mutableNode.__key === latestNode.__key)) {
            formatDevErrorMessage("$cloneWithProperties: ".concat(constructor.name, ".clone(node) (with type '").concat(constructor.getType(), "') did not return a node with the same key, make sure to specify node.__key as the last argument to the constructor"));
        }
        if (!(mutableNode.__parent === latestNode.__parent && mutableNode.__next === latestNode.__next && mutableNode.__prev === latestNode.__prev)) {
            formatDevErrorMessage("$cloneWithProperties: ".concat(constructor.name, ".clone(node) (with type '").concat(constructor.getType(), "') overrode afterCloneFrom but did not call super.afterCloneFrom(prevNode)"));
        }
    }
    return mutableNode;
}
 ...
 */ class AbstractCaret {
    ...
    splice(deleteCount, nodes) {
        let nodesDirection = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : 'next';
        ...
        for (const node of nodeIter){
            if (nodesToRemove.size > 0) {
                ...
                if (target) {
                    ...
                    if (!(target !== null)) {
                        formatDevErrorMessage("NodeCaret.splice: Underflow of expected nodesToRemove during splice (keys: ".concat(Array.from(nodesToRemove).join(' '), ")"));
                    }
                }
            } else {
                caret.insert(node);
...
class AbstractChildCaret extends AbstractCaret {
   ...
   */ getParentCaret() {
        let mode = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : 'root';
    ...
    constructor(...args){
        super(...args), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$swc$2f$helpers$2f$esm$2f$_define_property$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"])(this, "type", 'child');
    }
}
class ChildCaretFirst extends AbstractChildCaret {
    ...
    constructor(...args){
        super(...args), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$swc$2f$helpers$2f$esm$2f$_define_property$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"])(this, "direction", 'next');
    }
}
class ChildCaretLast extends AbstractChildCaret {
    ...
    constructor(...args){
        super(...args), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$swc$2f$helpers$2f$esm$2f$_define_property$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"])(this, "direction", 'previous');
    }
}
...
function $filterByMode(node) {
    let mode = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 'root';
...
class AbstractSiblingCaret extends AbstractCaret {
    ...
    constructor(...args){
        super(...args), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$swc$2f$helpers$2f$esm$2f$_define_property$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"])(this, "type", 'sibling');
    }
}
class AbstractTextPointCaret extends AbstractCaret {
    ...
    constructor(origin, offset){
        super(origin), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$swc$2f$helpers$2f$esm$2f$_define_property$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"])(this, "type", 'text');
        this.offset = offset;
    }
}
/**
 ...
 */ function $getTextNodeOffset(origin, offset) {
    ...
    if (numericOffset < 0 || numericOffset > size) {
        {
            formatDevErrorMessage("$getTextNodeOffset: invalid offset ".concat(String(offset), " for size ").concat(String(size), " at key ").concat(origin.getKey()));
        }
...
class CaretRangeImpl {
    ...
    iterNodeCarets() {
        let rootMode = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : 'root';
    ...
    constructor(anchor, focus, direction){
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$swc$2f$helpers$2f$esm$2f$_define_property$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"])(this, "type", 'node-caret-range');
        this.anchor = anchor;
        this.focus = focus;
...
class TextPointCaretSliceImpl {
    ...
    constructor(caret, distance){
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$swc$2f$helpers$2f$esm$2f$_define_property$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"])(this, "type", 'slice');
        this.caret = caret;
 ...
 */ function $comparePointCaretNext(a, b) {
    ...
    if (!(compare !== null)) {
        formatDevErrorMessage("$comparePointCaretNext: a (key ".concat(a.origin.getKey(), ") and b (key ").concat(b.origin.getKey(), ") do not have a common ancestor"));
    }
    switch(compare.type){
        case 'same':
            {
 ...
 */ function $caretFromPoint(point, direction) {
    ...
    if (type === 'text') {
        if (!$isTextNode(node)) {
            formatDevErrorMessage("$caretFromPoint: Node with type ".concat(node.getType(), " and key ").concat(key, " that does not inherit from TextNode encountered for text point"));
        }
        return $getTextPointCaret(node, direction, offset);
    }
    if (!$isElementNode(node)) {
        formatDevErrorMessage("$caretFromPoint: Node with type ".concat(node.getType(), " and key ").concat(key, " that does not inherit from ElementNode encountered for element point"));
    }
...
function $getAnchorCandidates(anchor) {
    let rootMode = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 'root';
 ...
 */ function $removeTextFromCaretRange(initialRange) {
    let sliceMode = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 'removeEmptySlices';
    ...
    {
        formatDevErrorMessage("$removeTextFromCaretRange: selection was lost, could not find a new anchor given candidates with keys: ".concat(JSON.stringify(anchorCandidates.map((n)=>n.origin.__key))));
    }
}
/**
 ...
 */ function $getAdjacentSiblingOrParentSiblingCaret(startCaret) {
    let rootMode = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 'root';
    let depthDiff = 0;
 ...
 */ function $splitAtPointCaretNext(pointCaret) {
    let { $copyElementNode = $copyNode, $splitTextPointCaretNext = $splitTextPointCaret, rootMode = 'shadowRoot', $shouldSplit = $alwaysSplit } = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
